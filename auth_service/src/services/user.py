...
class UserService(BaseService):
    ...
    async def create_user(self,
        ...
        if default_role:
            self.repo.session.add(
                UserRole(user_id=user.user_id, role_id=default_role.role_id)
            )
    ...
    async def get_login_history(
        self, user_id: str, params: Params
    ) -> Page[LoginHistory]:
    ...
    async def update_user(
        ...
        if update.old_password and update.new_password:
            if verify_password(update.old_password,
        ...
        if not updated:
            raise HTTPException(
                HTTPStatus.BAD_REQUEST,
                "No changes provided. "
                "Please specify username or password update.",
            )
