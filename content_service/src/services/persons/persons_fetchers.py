...
async def fetch_persons_list(search,
                             size: int = 1000,
...
async def fetch_persons_paginated(search,
                                  page: int,
                                  size: int) -> List[Person]:
    all_persons = await fetch_persons_list(search)
    return Paginator.paginate(all_persons, page, size, Person)


async def fetch_person_by_id(self,
                             person_uuid: UUID) -> Optional[Person]:
