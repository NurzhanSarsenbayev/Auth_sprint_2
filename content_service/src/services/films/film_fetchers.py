...
async def fetch_films_list(service,
                           page: int,
                           size: int,
                           sort: str) -> List[FilmShort]:
    resp = await service.search_index(
        "movies",
        all_films_query(page, size, sort))
...
async def fetch_short_film_by_name(
        service,
        query_str: str,
        page: int = 1,
        size: int = 10):
    resp = await service.search_index(
        "movies", search_films_query(query_str, page, size))
    return [parse_film_short(doc) for doc in resp["hits"]["hits"]]
